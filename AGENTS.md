...
| --- | --- |
| SubRip | `-c:s copy` out of the container, or the sidecar read where it lies. |
...
| ASS, TTML, PGS, VobSub | Refused, for now. |

**The staged filename and the extraction's `-c:s` have to agree.** `ffmpeg` picks its muxer from the extension, so a WebVTT track transcoded to SubRip and written to `cues.vtt` fails on a codec the muxer will not take — and the mistake is invisible in either half alone, which is why an e2e scenario covers it rather than only a unit test of the command.
